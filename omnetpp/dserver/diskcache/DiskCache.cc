...
void DiskCache::handleDiskAccessResp(PageRequest * resp){
	...
	// Not a problem if not found in the reqQ. - not triggered by requests.
...
#ifdef DEBUG
	printf("DiskCache: Write cache. [%ld,%ld]\n",start, end);
#endif
	...
	while(true){
		if(cachedp == NULL || addp->start < cachedp->start){ // ...{---[
			...
			if((!preop) || (preop && disk && !read) || (preop && !disk && read)){
				printf("[ERROR] DiskCache: cache increase operation not permitted!"
						"preop=%d, disk=%d, read=%d.\n", preop, disk, read);
				return;
			}
...
//					file->pr_head = tmp;
		...
		if(addp->start < cachedp->end){ // [___{---]...
			if(preop && disk && read){
				fprintf(stderr, "[ERROR] DiskCache: read the pages already in cache from disk. %d, %d, %d\n", preop, disk, read);
				fflush(stderr);
			...
			if(disk && !read){ // writing to disk
				if(tmp->PG_dirty == false){
					fprintf(stderr, "[ERROR] DiskCache: writing back pages which are not dirty.\n");
					return;
				}
				if(preop){ // before *
...
void DiskCache::printCache(int fid){
	pr_t * cachedp = files->find(fid)->second->pr_head;
	...
	while(true){
		...
		printf("[%ld %ld] C:%d DIRTY:%d WB:%d LCKED:%d REF:%d MOD:%d\n",
				...
				cachedp->PG_locked, cachedp->refered, cachedp->modified);
		cachedp = cachedp->next;
	}
...
int DiskCache::walkCache_countMissing(int fid, pr_t * addp){
	struct pr_t * cachedp;
	int miss_size = 0;
	// Set cachedp: the pointer to the cached page-range list of the file.
	...
	while(true){
		if(cachedp == NULL || addp->start < cachedp->start){ // ...{---[
			if(cachedp == NULL){
				miss_size += addp->end - addp->start;
				break;
			}
			else if(addp->end <= cachedp->start){ // ...{+++}___[ --> [+++]___[
				miss_size += addp->end - addp->start;
				break;
			}else{ // ...{+++[ --> ...[+++][
				miss_size += cachedp->start - addp->start;
				addp->start = cachedp->start; // Update addp to move forward.
			}
		}
		if(addp->start < cachedp->end){ // [___{---]...
			if(addp->start == cachedp->start){ // [{---]
				if(addp->end < cachedp->end){ // [{++}++] --> [+cachedp+][+tmp+]
					// All covered.
					break;
		...
		if(addp->start == addp->end) // addp size is zero.
			break;
		cachedp = cachedp->next;
	}
	return miss_size;
